...
class FirebaseDatabaseService {
    ...
    async getEvents() {
        try {
            ...
            const events = [];
            querySnapshot.forEach((doc) => {
                if (doc.id !== 'current') { // Skip the current event document
    ...
    async deleteEvent(eventId) {
        try {
            ...
            const attendanceRecords = await this.getAttendanceRecordsByEvent(eventId);
    ...
    async getCurrentEvent() {
        try {
            const doc = await window.firebaseDb.collection(this.eventsCollection).doc('current').get();
            if (doc.exists) {
    ...
    async setCurrentEvent(eventName) {
        try {
            await window.firebaseDb.collection(this.eventsCollection).doc('current').set({
                eventName: eventName,
                updatedAt: new Date()
            });
